...
class ProblemIndexSaveSettingInteractorTest {
    ...
    fun invoke() {
        ...
        val limit = 50
        val inputData = ProblemIndexSaveSettingUseCase.InputData(
        ...
        assertEquals(value, getList(SettingItems.PROBLEM_INDEX_VALUE).firstOrNull())
        assertEquals(column.name, getList(SettingItems.PROBLEM_INDEX_COLUMN).firstOrNull())
        assertEquals(orderBy.name, getList(SettingItems.PROBLEM_INDEX_ORDER_BY).firstOrNull())
        assertEquals(limit.toString(), getList(SettingItems.PROBLEM_INDEX_LIMIT).firstOrNull())
    }
    ...
    private lateinit var useCase: ProblemIndexSaveSettingUseCase

    @Before
    fun beforeEach() {
        dbTestUtil.setUp()
        useCase = ProblemIndexSaveSettingInteractor(
